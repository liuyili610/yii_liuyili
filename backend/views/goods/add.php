...
echo $form->field($goods, 'logo')->widget(\manks\FileInput::className(),['clientOptions' => [ 'server' => \yii\helpers\Url::to(['brand/upload'])]]);
...
echo $form->field($goods,'is_on_sale')->inline()->radioList(['1'=>'上架','2'=>'不上架'],['value'=>1]);
//echo $form->field($goods,'status')->textInput()->label('是否删除');



//上传多图
echo $form->field($goods, 'imgFile')->widget('manks\FileInput', [
    'clientOptions' => [
        'pick' => [
            'multiple' => true,
        ],
